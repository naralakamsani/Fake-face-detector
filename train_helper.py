...
def create_model(arch='alexnet', input_units=9216, hidden_units=256, output_units=2):
...
def train(model, trainloader, validloader, print_every=50, criterion=nn.NLLLoss(), lr=0.001, epochs=1, gpu=True):
    if gpu:
        device = 'cuda'
    else:
        device = 'cpu'
        ...
        for inputs, labels in trainloader:
            ...
            if steps % print_every == 0:
                ...
                print(f"Epoch {epoch+1}/{epochs}.. "
                      f"Step {int(steps/print_every)}/{int(len(trainloader)/print_every)}.. "
